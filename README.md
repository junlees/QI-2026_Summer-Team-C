...
│   ├── login.html          # 로그인 폼 (아직 실제 인증 없음, 제출하면 index.html로)
...
```
시작 화면 → (로그인 또는 게스트로 계속) → 홈 → 작물 선택 → 사진 업로드 → AI 진단 결과 확인
```

홈/작물 선택/진단 결과 3개 화면은 상단 헤더(로고 + Home·Diagnose·History·Profile
탭)를 공통으로 씁니다. 예전에는 이 네비게이션이 홈 화면 하단에만 있었는데, 상단
헤더로 옮기고 전체 화면에 통일해서 적용했습니다.
